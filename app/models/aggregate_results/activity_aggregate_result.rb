class ActivityAggregateResult < AggregateResult
  ...
  def calculate_weekly_average(date, week_day_score, new_value)
    ...
    last_updated_str = self.scores ? self.scores['last_updated'] : (Date.current - 5.days).to_s
    last_updated = Date.parse(last_updated_str)
    last_value = self.scores ? self.scores['last_value'].to_i : 0
    total = week_day_score['total'].to_i
    data_points = week_day_score['data_points'].to_i
    ...
    most_steps = new_value > week_day_score['most_steps'].to_i ? new_value : week_day_score['most_steps'].to_i
