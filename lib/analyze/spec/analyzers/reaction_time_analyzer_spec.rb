...
module TidepoolAnalyze
  module Analyzer
    describe ReactionTimeAnalyzer do 
      before(:all) do
        events_json = IO.read(File.expand_path('../../fixtures/aggregate_reaction_time.json', __FILE__))
        @simple_user_events = JSON.parse(events_json)[0]['events']
        @complex_user_events = JSON.parse(events_json)[1]['events']
      end

      it 'should record the test end time and greater than start_time' do
        reaction_time_analyzer = ReactionTimeAnalyzer.new(@simple_user_events, nil)
      ...
      it 'calculates the result for the complex reaction_time' do
        reaction_time_analyzer = ReactionTimeAnalyzer.new(@complex_user_events, nil)
        result = reaction_time_analyzer.calculate_result
        result.should_not be_nil
